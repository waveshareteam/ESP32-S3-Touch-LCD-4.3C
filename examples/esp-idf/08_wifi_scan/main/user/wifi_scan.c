...
void wifi_scan(void)
{
    ...
    ESP_LOGI(TAG, "Max AP number ap_info can hold = %u", number);  // Log the max AP number that can be stored
    ...
    ESP_LOGI(TAG, "Total APs scanned = %u, actual AP number ap_info holds = %u", ap_count, number);  // Log total and actual scanned APs

    // Loop through each AP and log details
    for (int i = 0; i < number; i++) {
        ESP_LOGI(TAG, "SSID \t\t%s", ap_info[i].ssid);  // Log SSID (network name)
        ESP_LOGI(TAG, "RSSI \t\t%d", ap_info[i].rssi);  // Log RSSI (signal strength)
        ...
        ESP_LOGI(TAG, "Channel \t\t%d", ap_info[i].primary);  // Log channel number
    }
}
